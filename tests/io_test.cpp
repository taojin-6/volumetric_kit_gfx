...
// fixture skips the glTF tests rather than failing them.
bool have_fixture() {
...
TEST(GltfLoader, MissingFileReportsError) {
  ...
  std::optional<assets::Model> model =
      io::load_gltf("/no/such/dir/does_not_exist.glb", &err);
...
TEST(GltfLoader, UnsupportedExtensionReportsError) {
  std::string err;
  std::optional<assets::Model> model = io::load_gltf("model.fbx", &err);
  EXPECT_FALSE(model.has_value());
