...
# vg_embed_shaders(<target> [SYMBOL_PREFIX <prefix>] SHADERS <file>...)
#
# Compiles each GLSL shader to SPIR-V (via vg_compile_shaders) and embeds the
...
# runtime files. For a shader <stem>.<stage> (e.g. model.vert) the generated
...
# SYMBOL_PREFIX defaults to `vg_`. Safe to call more than once per target.
function(vg_embed_shaders target)
  cmake_parse_arguments(ARG "" "SYMBOL_PREFIX" "SHADERS" ${ARGN})
  if(NOT ARG_SHADERS)
    message(FATAL_ERROR "vg_embed_shaders(${target}): no SHADERS given")
  endif()
  if(NOT ARG_SYMBOL_PREFIX)
    set(ARG_SYMBOL_PREFIX "vg_")
  endif()
  ...
  foreach(_src IN LISTS ARG_SHADERS)
    get_filename_component(_name "${_src}" NAME) # e.g. model.vert
    string(REPLACE "." "_" _stem "${_name}") # e.g. model_vert
  ...
  # name.
  get_property(_seq GLOBAL PROPERTY _vg_embed_set_seq)
  if(NOT _seq)
    set(_seq 0)
  endif()
  math(EXPR _seq "${_seq} + 1")
  set_property(GLOBAL PROPERTY _vg_embed_set_seq "${_seq}")
