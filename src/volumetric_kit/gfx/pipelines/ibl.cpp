...
std::vector<uint32_t> pack_cube_rgba16f(uint32_t base_size, uint32_t mips,
                                        Gen gen) {
  std::vector<std::future<std::vector<glm::vec4>>> blocks;
  blocks.reserve(static_cast<size_t>(mips) * 6);
  for (uint32_t m = 0; m < mips; ++m) {
    const uint32_t size = (base_size >> m) > 0 ? (base_size >> m) : 1u;
    for (uint32_t f = 0; f < 6; ++f) {
      blocks.push_back(
          std::async(std::launch::async, gen, m, static_cast<int>(f), size));
    }
  }
  std::vector<uint32_t> data;
  for (std::future<std::vector<glm::vec4>>& block : blocks) {
    const std::vector<glm::vec4> face = block.get();
...
// Integrate the full LUT into packed RG16F texels (1 uint32 each), row bands
// in parallel: each band writes a disjoint row range with unchanged per-texel
...
std::vector<uint32_t> integrate_brdf_lut(uint32_t size, uint32_t samples) {
  ...
  const uint32_t threads =
      std::max(1u, std::min(std::thread::hardware_concurrency(), size));
  const uint32_t rows_per_band = (size + threads - 1) / threads;
  std::vector<std::future<void>> bands;
  bands.reserve(threads);
  for (uint32_t begin = 0; begin < size; begin += rows_per_band) {
    const uint32_t end = std::min(begin + rows_per_band, size);
    bands.push_back(
        std::async(std::launch::async, [&lut, size, samples, begin, end] {
          for (uint32_t y = begin; y < end; ++y) {
            for (uint32_t x = 0; x < size; ++x) {
              lut[y * size + x] = glm::packHalf2x16(brdf_integrate(
                  (x + 0.5f) / size, (y + 0.5f) / size, samples));
            }
          }
        }));
  }
...
Result<Texture> bake_brdf_lut(UploadBatch& batch, uint32_t size,
  ...
  const std::vector<uint32_t> lut = integrate_brdf_lut(size, samples);
...
Result<IblMaps> bake_ibl(const Device& device, Allocator& allocator,
  ...
  const std::vector<uint32_t> irradiance = pack_cube_rgba16f(
